import { IFAQ, AddFaqModel } from './faq.types'
...
export const faqApi = createApi({
	...
	endpoints: build => ({
		getFaq: build.query<IFAQ[], number>({
			query: () => `getFaq`,
		}),
		addFaq: build.query<IFAQ[], string>({
			query: (title: string = '', text: string = '', userId: number = 0) =>
				`getFaq?title=${title}&text=${text}&userId=${userId}`,
		}),
		editFaq: build.query<IFAQ[], number>({
			query: () => `getFaq`,
		}),
		deleteFaq: build.query<IFAQ[], number>({
			query: () => `getFaq`,
		}),
	}),
})
